	...
	if (Meteor.isClient) {
		...
		that.getPreparedInputs = function ( formName ) {
			...
			_(inputs).each( function( input ) {
				...
				// String should no be empty!
				if ((input.valType === 'string' || input.fieldType === 'text') && input.required) {
					if (value.length < 1)
						throw new Error( input.name + ' is required but not set!');
				}

				// Number should be numbers
				if (input.valType === 'number') {
				...
				if (input.valType === 'date') {
					// There should be a value!
					if (!value)
						throw new Error( input.name + ' is required not set!');
